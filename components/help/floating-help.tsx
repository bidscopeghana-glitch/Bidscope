...
export function FloatingHelp() {
  ...
  return <div className={`bidscope-help ${open ? "is-open" : ""}`}>
    {open && <div ref={panel} className="bidscope-help-panel" role="dialog" aria-label={mode === "voice" ? "Taleh, BidScope AI receptionist" : "BidScope Help Assistant"} aria-modal="false">
      ...
      <nav className="bidscope-help-modes" aria-label="Choose help or Taleh"><button type="button" className={mode === "help" ? "active" : ""} aria-pressed={mode === "help"} onClick={() => setMode("help")}><HelpCircle size={16}/> Help chat</button><button type="button" className={mode === "voice" ? "active" : ""} aria-pressed={mode === "voice"} onClick={() => setMode("voice")}><Mic size={16}/> Call Taleh</button></nav>
    ...
    <button className="bidscope-floating-help" onClick={() => setOpen(value => !value)} aria-label={open ? "Close BidScope help and Taleh" : "Open BidScope help and Taleh"} aria-expanded={open}>{open ? <X size={22}/> : <HelpCircle size={22}/>}<span>{open ? "Close" : "Help & Taleh"}</span></button>
  </div>;
}
