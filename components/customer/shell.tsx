...
import { Home,Search,Globe2,Bookmark,BriefcaseBusiness,Building2,ChartNoAxesCombined,FileText,CalendarDays,Bell,Settings,HelpCircle,PanelLeftClose,PanelLeftOpen,LogOut,ChevronRight,Sparkles,History,Eye,Menu,X,ArrowUpRight,SlidersHorizontal,Compass } from "lucide-react";
...
export function CustomerShell({children}:{children:React.ReactNode}){
 ...
 const notifications=useData<{data:Notice[];unreadCount:number}>(session?"/api/notifications":null);
 ...
 <div className="cc-body"><header className="cc-topbar"><button className="cc-mobile-menu" aria-label="Open navigation" onClick={()=>setMobile(true)}><Menu size={21}/></button><button className="cc-global-search" onClick={()=>setSearch(true)}><Search size={17}/><span>Search tenders, buyers, sectors, keywords or reference numbers…</span><kbd>⌘ / Ctrl K</kbd></button><div className="cc-top-actions"><Link href="/customer/ai" aria-label="Ask BidScope AI"><Sparkles size={18}/><span>Ask AI</span></Link><Link href="/customer/notifications" aria-label={`Notifications${notifications.data?`, ${notifications.data.unreadCount} unread`:""}`}><Bell size={18}/>{!!notifications.data?.unreadCount&&<i className="cc-unread"/>}</Link><Link href="/customer/help" aria-label="Help"><HelpCircle size={18}/></Link><Link href="/customer/profile" aria-label="Your profile" className="cc-avatar">{name.slice(0,1).toUpperCase()}</Link></div></header>
