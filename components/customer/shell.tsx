...
export function CustomerShell({children}:{children:React.ReactNode}){
 ...
 <div className="cc-body"><header className="cc-topbar"><button className="cc-mobile-menu" aria-label="Open navigation" onClick={()=>setMobile(true)}><Menu size={21}/></button><button className="cc-global-search" onClick={()=>setSearch(true)}><Search size={17}/><span>Search tenders, buyers, sectors, keywords or reference numbers…</span><kbd>⌘ / Ctrl K</kbd></button><div className="cc-top-actions"><Link href="/customer/ai" aria-label="Open AI Tender Evaluation"><Sparkles size={18}/><span>Evaluate tender</span></Link><Link href="/customer/notifications" aria-label={`Notifications${notifications.data?`, ${notifications.data.unreadCount} unread`:""}`}><Bell size={18}/>{!!notifications.data?.unreadCount&&<i className="cc-unread"/>}</Link><Link href="/customer/help" aria-label="Open Help Assistant"><HelpCircle size={18}/></Link><Link href="/customer/profile" aria-label="Your profile" className="cc-avatar">{name.slice(0,1).toUpperCase()}</Link></div></header>
