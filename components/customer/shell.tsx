...
export function CustomerShell({children}:{children:React.ReactNode}){
 ...
 <nav className="cc-nav">{navGroups.map((group,index)=><div key={index} className="cc-nav-group">{group.title&&<p>{group.title}</p>}{group.links.map(([label,href,Icon])=><Link key={href} title={label} aria-current={selected===href?"page":undefined} href={href} onClick={()=>setMobile(false)}><Icon size={16}/><span>{label}</span>{selected===href&&<i/>}</Link>)}{index===1?<CountryNavigation close={()=>setMobile(false)}/>:null}</div>)}</nav>
