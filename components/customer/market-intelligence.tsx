...
export function MarketIntelligence() {
  ...
  const mostActive = data.sectors[0];
  const fastest = data.sectors.filter(item => item.previous >= 3 && item.recent >= 3).sort((a,b) => (change(b.recent,b.previous) || 0) - (change(a.recent,a.previous) || 0))[0];
  ...
  const cards = [
    ...
    ["Fastest growing sector", fastest?.name || "Insufficient history"],
    ["Most active sector", mostActive?.name || "Not enough data"],
