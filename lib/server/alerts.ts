...
export function matchesAlert(rule: AlertRuleRecord, opportunity: AlertOpportunityRecord, now = new Date()) {
  const haystack = normalized(`${opportunity.title} ${opportunity.summary} ${opportunity.description} ${opportunity.sectors.join(" ")} ${opportunity.region || ""}`);
  ...
  if (rule.sectors.length && !rule.sectors.some((sector) => opportunity.sectors.map(normalized).includes(normalized(sector)))) return false;
  if (rule.regions.length && !rule.regions.map(normalized).includes(normalized(opportunity.region))) return false;
  if (rule.buyer_ids.length && (!opportunity.buyer_id || !rule.buyer_ids.includes(opportunity.buyer_id))) return false;
  ...
  if (rule.deadline_days_min != null || rule.deadline_days_max != null) {
    ...
    const days = Math.ceil((new Date(opportunity.deadline).getTime() - now.getTime()) / 86_400_000);
