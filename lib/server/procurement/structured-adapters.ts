...
function documentLinks(base: string, html: string) { return [...new Set([...html.matchAll(/href=["']([^"']+)["']/gi)].map((item) => absolute(base, item[1])).filter((url) => /\.pdf(?:$|\?)|download|document/i.test(url)))]; }
async function mapLimited<T, R>(values: T[], concurrency: number, mapper: (value: T) => Promise<R>) { const output: R[] = []; for (let index = 0; index < values.length; index += concurrency) output.push(...await Promise.all(values.slice(index, index + concurrency).map(mapper))); return output; }
...
export class GhanaRoadsAdapter extends BaseAdapter {
  ...
  async fetchOpportunities() { enforceSourceRateLimit(this.slug, 20); const response = await fetchWithRetry(this.endpoint, { headers: { Accept: "application/json", "User-Agent": "BidScopeGhana/1.0" } }); const rows = z.array(RecordSchema).parse(await response.json()); return mapLimited(rows, 3, async (row) => { const content = String((row.content as Raw)?.rendered || ""); const pdf = content.match(/href=["']([^"']+\.pdf(?:\?[^"']*)?)/i)?.[1] || null; const noticeText = pdf ? await extractOfficialPdfText(pdf).catch(() => "") : ""; return { ...row, documentUrl: pdf, noticeText }; }); }
  async normaliseOpportunity(raw: Raw) { const content = String((raw.content as Raw)?.rendered || ""); const title = text((raw.title as Raw)?.rendered || ""); const pageBody = text(content); const noticeText = text(raw.noticeText, 50_000); const body = noticeText || pageBody; const deadline = firstDate(body.match(/(?:deadline|closing date|submission[^.]{0,30})(?:\s*(?:is|:|-))?\s*([A-Z][a-z]+\s+\d{1,2},?\s+\d{4}(?:[^.;]{0,20})?)/i)?.[1]); const pdf = String(raw.documentUrl || "") || null; return normalize({ title, summary: body.slice(0, 1_000), description: body, buyer_name: "Ministry of Roads and Highways, Ghana", country: "Ghana", country_code: "GH", published_at: firstDate(raw.date_gmt, raw.date), deadline_at: deadline, source_name: "Ministry of Roads and Highways", source_type: "OPEN_API", external_opportunity_id: String(raw.id), external_reference: body.match(/(?:reference|ref\.?|contract no\.?)[\s:#-]*([A-Z0-9/_.-]{5,})/i)?.[1] || null, official_source_url: String(raw.link), documents_url: pdf || String(raw.link), funding_source: "Government of Ghana", source_details: { officialDocument: pdf, extraction: noticeText ? "Official tender PDF" : "Official procurement page" }, raw_payload: raw }); }
...
abstract class RegionalHtmlAdapter extends BaseAdapter {
  ...
  async fetchOpportunities() { enforceSourceRateLimit(this.slug, 10); const response = await fetchWithRetry(this.endpoint, { headers: { Accept: "text/html", "User-Agent": "BidScopeGhana/1.0 (+https://www.bidscopeghana.com)" } }); const rows = this.parse(await response.text()); return mapLimited(rows, 4, async (row) => { try { const url = String(row.url); const html = await (await fetchWithRetry(url, { headers: { Accept: "text/html", "User-Agent": "BidScopeGhana/1.0" } }, 2)).text(); const documents = documentLinks(url, html); const noticeText = documents[0] && /\.pdf(?:$|\?)/i.test(documents[0]) ? await extractOfficialPdfText(documents[0]).catch(() => "") : ""; return { ...row, detailHtml: html, detailText: decode(html), documents, noticeText }; } catch { return row; } }); }
  async normaliseOpportunity(raw: Raw) { const deadline = firstDate(raw.deadline); const noticeText = text(raw.noticeText, 50_000); const detailText = text(raw.detailText, 50_000); const description = noticeText || detailText || String(raw.summary || ""); const documents = Array.isArray(raw.documents) ? raw.documents.filter((value): value is string => typeof value === "string") : []; return normalize({ title: String(raw.title), summary: description.slice(0, 1_000), description, buyer_name: this.sourceName, country: this.country, country_code: String(raw.countryCode || "ZZ"), published_at: firstDate(raw.published), deadline_at: deadline, source_name: this.sourceName, source_type: "STRUCTURED_WEB", external_opportunity_id: String(raw.id || raw.url), external_reference: String(raw.reference || "") || null, official_source_url: String(raw.url), official_tender_url: String(raw.url), documents_url: documents[0] || String(raw.url), funding_source: this.sourceName, eligibility_text: String(raw.eligibility || "") || null, source_details: { documents, extraction: noticeText ? "Official tender PDF" : detailText ? "Official notice page" : "Official listing" }, raw_payload: raw }); }
  getOfficialUrl(raw: Raw) { return String(raw.url || this.endpoint); }
}
