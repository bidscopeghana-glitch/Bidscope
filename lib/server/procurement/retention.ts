...
type SupplierDocument={id:string;document_type:string;title:string;expires_at:string|null;verification_status:string};
...
export function calculateRetentionAssessment(profile:Organization, opportunity:RetentionOpportunity, documents:SupplierDocument[]=[]):RetentionAssessment{
 ...
 const reasons=[...base.reasons];const concerns:string[]=[];const now=Date.now();const deadline=opportunity.deadline_at?Date.parse(opportunity.deadline_at):null;
 ...
 const activeDocs=documents.filter(d=>!d.expires_at||Date.parse(`${d.expires_at}T23:59:59Z`)>now);
 const documentReadiness=documents.length?Math.round(activeDocs.filter(d=>d.verification_status!=="rejected").length/documents.length*100):null;
...
export function calculateBusinessReadiness(profile:Organization,documents:SupplierDocument[]=[]){
 const now=Date.now();const validDocs=documents.filter(d=>d.verification_status!=="rejected"&&(!d.expires_at||Date.parse(`${d.expires_at}T23:59:59Z`)>now));
 const categories={
...
export async function refreshOrganizationRetention(userId:string){
 ...
 const[{data:opportunities},{data:documents},{data:feedback},{data:upcomingOpportunities}]=await Promise.all([
  ...
  supabaseRest<SupplierDocument[]>(`supplier_documents?select=id,document_type,title,expires_at,verification_status&organization_id=eq.${organization.id}`),
...
export async function assessOpportunityForUser(userId:string,opportunityId:string,persistDecision=false){
 ...
 const[{data:opportunities},{data:documents}]=await Promise.all([supabaseRest<RetentionOpportunity[]>(`procurement_opportunities?select=*&id=eq.${opportunityId}&limit=1`),supabaseRest<SupplierDocument[]>(`supplier_documents?select=id,document_type,title,expires_at,verification_status&organization_id=eq.${organization.id}`)]);
