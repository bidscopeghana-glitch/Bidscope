...
export const CORE_VERIFICATION_CHECKS = ["company_registration", "company_name_match", "registration_number_match", "document_integrity", "manual_review"] as const;
export const PPA_CHECKS = ["ppa_supplier_registration", "ppa_barred_supplier"] as const;
export const VERIFIED_CHECKS = [...CORE_VERIFICATION_CHECKS, ...PPA_CHECKS] as const;
...
export function mayApprove(request: {requested_level:RequestedLevel;payment_status:string}, checks: VerificationCheck[]) {
  ...
  return requiredChecks(request.requested_level).every(type => checks.some(check => check.check_type === type && (check.status === "passed" || (!CORE_VERIFICATION_CHECKS.includes(type as typeof CORE_VERIFICATION_CHECKS[number]) && check.status === "not_applicable"))));
}
