...
export async function processCustomerSearchAlerts(){
  const {data:searches}=await supabaseRest<Array<{id:string;user_id:string;name:string;filters:Record<string,unknown>;frequency:"instant"|"daily"|"weekly";last_notified_at:string|null;created_at:string}>>("customer_saved_searches?select=*&alerts_enabled=eq.true&order=last_notified_at.asc.nullsfirst&limit=50");
  let generated=0;
