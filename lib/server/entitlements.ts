...
export async function getEntitlement(organizationId:string,actor?:AuthenticatedUser){
 ...
 const subscription=subscriptions[0]||null;const manualGrant=grants[0]||null;const premium=Boolean(manualGrant||(subscription&&subscription.plan_code.toLowerCase()==="premium"&&periodValid(subscription)));
