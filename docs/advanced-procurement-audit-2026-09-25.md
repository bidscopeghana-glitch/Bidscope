...
| 27 Analytics | Partial: procurement dashboard and event data | Buyer Reports paginate tenant-scoped records with bounded limit, date/category filters, response/cycle metrics and currency-separated finalised spend. Supplier Managed Tender Performance adds organisation-scoped drafts, submissions, current shortlist/interview status, confirmed awards, completion/turnaround, category/monthly activity and currency-separated finalised award value. | PARTIAL: external tender views/matches/outcomes, verified/SME participation, region, evaluation duration, contract performance and full trend comparisons outstanding. CSV export remains disabled per explicit user request. |
