...
export async function GET(request:Request){try{const{user}=await requireUser(request);const organizationId=z.string().uuid().parse(new URL(request.url).searchParams.get("organizationId"));await requireOrganizationMember(user.id,organizationId);const[entitlement,{data:transactions}]=await Promise.all([getEntitlement(organizationId,user),supabaseRest(`payment_transactions?select=id,reference,billing_plan_code,amount_minor,currency,payment_kind,status,channel,paid_at,created_at,failure_reason&organization_id=eq.${organizationId}&order=created_at.desc&limit=100`)]);return Response.json({data:{entitlement,transactions}});}catch(error){return apiErrorResponse(error);}}
