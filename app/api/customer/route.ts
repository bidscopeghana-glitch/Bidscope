...
export async function GET(request:Request) {
  try {
    ...
    if(resource==="countries") {
      ...
      const iso3:Record<string,string>={AUT:"Austria",BEL:"Belgium",BGR:"Bulgaria",CHE:"Switzerland",CYP:"Cyprus",CZE:"Czechia",DEU:"Germany",DNK:"Denmark",ESP:"Spain",EST:"Estonia",FIN:"Finland",FRA:"France",GBR:"United Kingdom",GRC:"Greece",HRV:"Croatia",HUN:"Hungary",IRL:"Ireland",ISL:"Iceland",ITA:"Italy",LIE:"Liechtenstein",LTU:"Lithuania",LUX:"Luxembourg",LVA:"Latvia",MKD:"North Macedonia",MLT:"Malta",NLD:"Netherlands",NOR:"Norway",POL:"Poland",PRT:"Portugal",ROU:"Romania",SRB:"Serbia",SVK:"Slovakia",SVN:"Slovenia",SWE:"Sweden",TUR:"Turkey",UKR:"Ukraine"};
      const totals=new Map<string,{country:string;filter:string;code:string;count:number}>();
      for(const row of data){const raw=row.country?.trim();if(!raw)continue;const candidates=raw.split(",").map(value=>value.trim()).filter(Boolean);const parts=candidates.length>1&&candidates.every(value=>/^[A-Z]{3}$/.test(value))?[...new Set(candidates)]:[raw];for(const filter of parts){const country=iso3[filter.toUpperCase()]||filter;const key=country.toLocaleLowerCase("en");const current=totals.get(key);if(current)current.count++;else totals.set(key,{country,filter,code:row.country_code||"",count:1});}}
