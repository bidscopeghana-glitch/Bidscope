...
export async function GET(request:Request) {
  try {
    ...
    if(resource==="countries") {
      ...
      for(const row of data){const raw=row.country?.trim();if(!raw)continue;const parts=[...new Set(raw.split(",").map(value=>value.trim()).filter(Boolean))];const filter=parts.length===1?parts[0]:raw;const country=parts.length===1?(iso3[parts[0].toUpperCase()]||parts[0]):parts.map(value=>iso3[value.toUpperCase()]||value).join(", ");const key=country.toLocaleLowerCase("en");const current=totals.get(key);if(current)current.count++;else totals.set(key,{country,filter,code:row.country_code||"",count:1});}
