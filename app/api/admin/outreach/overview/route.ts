...
export async function GET(request:Request){try{await requireSuperAdmin(request);const now=new Date(),month=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),1)).toISOString();const[total,emails,phones,newThisMonth,hot,high,medium,low,campaigns,sent,delivered,clicks,unsubscribes,bounces]=await Promise.all([
  ...
  const{data:countries}=await supabaseRest<Array<{country_code:string|null}>>("prospects?select=country_code&limit=10000");const{data:industries}=await supabaseRest<Array<{industry:string|null}>>("prospects?select=industry&limit=10000");const tally=(values:Array<string|null>)=>Object.entries(values.reduce<Record<string,number>>((acc,value)=>{const key=value||"Unknown";acc[key]=(acc[key]||0)+1;return acc;},{})).sort((a,b)=>b[1]-a[1]).slice(0,12).map(([name,value])=>({name,value}));
