...
const actions=z.discriminatedUnion("action",[
  z.object({action:z.literal("check"),requestId:z.string().uuid(),checkType:z.string().min(2).max(80),status:z.enum(["passed","failed","needs_review","not_applicable"]),source:z.string().max(500).optional(),evidenceReference:z.string().max(500).optional(),notes:z.string().max(5000).optional()}),
  ...
  z.object({action:z.literal("settings"),enhancedPriceMinor:z.number().int().min(0).max(100000000),validityMonths:z.number().int().min(1).max(60)}),
...
export async function GET(request:Request){
  try{
    ...
    const [{data:organizations},{data:checks},{data:documents}]=await Promise.all([
...
export async function POST(request:Request){
  try{
    ...
    if(input.action==="settings"){
      await supabaseRest("supplier_verification_settings?id=eq.true",{method:"PATCH",body:JSON.stringify({enhanced_price_minor:input.enhancedPriceMinor,validity_months:input.validityMonths,updated_by:user.id,updated_at:new Date().toISOString()})});
    ...
    if(input.action==="check"){
      ...
      const {data:existing}=await supabaseRest<Array<{id:string;status:string}>>(`supplier_verification_checks?select=id,status&request_id=eq.${application.id}&check_type=eq.${encodeURIComponent(input.checkType)}&limit=1`);
      ...
      if(input.status==="passed"&&!input.source?.trim())throw new ApiError(400,"Record the verification source before passing a check.","verification_source_required");
      await supabaseRest(`supplier_verification_checks?id=eq.${existing[0].id}`,{method:"PATCH",body:JSON.stringify({status:input.status,source:input.source||null,evidence_reference:input.evidenceReference||null,notes:input.notes||null,reviewed_by:user.id,reviewed_at:new Date().toISOString(),automated:false})});
      await verificationAudit(application,user.id,"check_reviewed",{check_type:input.checkType,previous_status:existing[0].status,status:input.status,source:input.source||null,evidence_reference:input.evidenceReference||null});
      return Response.json({data:{saved:true}});
    }
