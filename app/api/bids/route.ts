...
export async function POST(request: Request) {
  try {
    ...
    if (input.organizationId) await requireOrganizationMember(user.id, input.organizationId);
    ...
    const body = { user_id: user.id, organization_id: input.organizationId || null, opportunity_id: input.opportunityId, status: input.status, submission_reference: input.submissionReference || null, outcome: input.outcome || null, notes: input.notes, ...timestamps };
