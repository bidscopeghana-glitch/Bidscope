...
import { documentValidity } from "@/lib/document-passport";
...
export async function GET(request:Request){
  try{
    ...
    type Row={id:string;uploaded_by:string;issued_at:string|null;expires_at:string|null;verification_status:string;[key:string]:unknown};
    const{data}=await supabaseRest<Row[]>(`supplier_documents?select=id,document_type,title,storage_path,source_url,issued_at,expires_at,verification_status,created_at,updated_at,uploaded_by,metadata&organization_id=eq.${organizationId}&order=created_at.desc`);
    ...
    return Response.json({data:data.map(item=>({...item,validity:documentValidity(item),uploader:people.get(item.uploaded_by)?.full_name||people.get(item.uploaded_by)?.email||"Workspace member",can_manage:canManageAll||item.uploaded_by===user.id}))},{headers:{"Cache-Control":"private, no-store"}});
