...
export async function POST(request:Request){
  try{
    ...
    await supabaseRest("meeting_oauth_states",{method:"POST",body:JSON.stringify({state_hash:createHash("sha256").update(state).digest("hex"),user_id:user.id,return_path:"/customer/meetings",expires_at:new Date(Date.now()+10*60_000).toISOString()})});
