...
export async function POST(request:Request){
  try{
    ...
    await Promise.all([...memberIds.map((id)=>createNotification({userId:id,organizationId:membership.organization_id,type:"system",title:"Meeting invitation",message:`You have been invited to ${input.title}.`,relatedEntityType:"meeting",relatedEntityId:meeting.id,relatedUrl,priority:"high",frequencyOverride:"instant",dedupeKey:stableDedupe(["meeting-invite",meeting.id,id])})),...guestTokens.map(({email,token})=>sendGuestInvite(email,input.title,startsAt.toISOString(),token))]);
