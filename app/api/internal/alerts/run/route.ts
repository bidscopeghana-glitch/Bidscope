...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ matched: candidates.length, queued: outcomes.filter((value) => value === "queued").length, sent: outcomes.filter((value) => value === "sent").length, failed: outcomes.filter((value) => value === "failed").length });
  } catch (error) {
