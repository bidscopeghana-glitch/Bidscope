...
type Result = { mode: string; recordsRead: number; uniqueOcids: number; active: number; closed: number; awarded: number; malformed: number; inserted: number; updated: number; duplicates: number; rejected: number; errors: number; changed?: boolean; lastModified?: string };
...
export function GhanepsOcdsCard() {
  ...
  return <section className="rounded-2xl border border-[#cbded4] bg-white p-5 shadow-sm" aria-labelledby="ocds-heading">
    ...
    {result && <p className="mt-4 rounded-xl bg-[#f3f8f4] p-4 text-sm">{result.mode}: {result.recordsRead ?? 0} records · {result.uniqueOcids ?? 0} OCIDs · {result.active ?? 0} current active · {result.closed ?? 0} other/closed · {result.awarded ?? 0} awarded/contracted · {result.inserted ?? 0} inserted · {result.updated ?? 0} updated · {result.duplicates ?? 0} duplicate links · {result.malformed ?? 0} malformed · {result.errors ?? 0} errors{result.changed !== undefined ? ` · dataset ${result.changed ? "changed" : "unchanged"}` : ""}</p>}
