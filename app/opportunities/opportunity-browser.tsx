...
type LoadedResult = { result: Result; showingArchive: boolean };
...
async function fetchOpportunityResults(next: URLSearchParams): Promise<LoadedResult> {
  ...
  const requestedClosed = next.get("status")?.toLowerCase() === "closed";
  if (json.data.length || requestedClosed) return { result: json, showingArchive: false };

  const archiveParams = new URLSearchParams(next);
  archiveParams.set("status", "closed"); archiveParams.set("sort", "newest");
  const archiveResponse = await fetch(`/api/opportunities?${archiveParams.toString()}`);
  const archive = await archiveResponse.json() as Result & { error?: string };
  if (!archiveResponse.ok) throw new Error(archive.error || "Past opportunities could not be loaded.");
  return { result: archive, showingArchive: archive.data.length > 0 };
}
...
export function OpportunityBrowser() {
  ...
  async function load(next = params) {
    ...
    try {
      const loaded = await fetchOpportunityResults(next);
      setResult(loaded.result); setShowingArchive(loaded.showingArchive);
    }
  ...
  useEffect(() => {
    void fetchOpportunityResults(new URLSearchParams())
      .then((loaded) => { setResult(loaded.result); setShowingArchive(loaded.showingArchive); })
  ...
  {showingArchive && <div role="status" className="mt-5 rounded-2xl border border-[#116149]/15 bg-[#eaf2ed] px-5 py-4 text-sm leading-6 text-[#315b4e]"><strong>No open notices matched.</strong> Showing relevant past opportunities instead, clearly marked closed, so you can research buyers and previous procurement patterns.</div>}
